...
exports.run = async function ({ client, msg, args }) {
    ...
    const embed = {
        color       : client.config.options.embedColour,
        title       : `<a:MusicNotes:489099234461745163> Now Playing`,
        description : `
        **[${track.title}](${track.permalink})**
        ${timeParser.formatSeconds(current.playTime / 1000)}/${timeParser.formatSeconds(track.duration / 1000)}`,
        fields: [
            {
                name: 'Song Queue',
                value: audioPlayer.queue.slice(startQueue, endQueue).map((item, i) => `${startQueue + i + 1}. ${item.title} - Requested By ${client.users.has(item.req) ? client.users.get(item.req).username : 'Unknown'}`).join('\n')
            }
        ],
        footer: {
            text: `Page ${page}/${maxPage} || my!queue <page>`
        }
    };
